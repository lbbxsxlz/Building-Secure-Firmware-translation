...
为了提高符号执行和模糊测试的效率，我们需要考虑如何编写程序：

1) 我们从编写小型函数并测试这些函数开始。小型函数便于代码审查和测试。例如，在EDK II UEFI变量驱动程序MdeModulePkg/Universal/Variable/RuntimeDxe/Variable.c中，UpdateVariable()例程包含600行代码和65个“if”分支。其中一个“if”分支遗漏了“else”分支处理，导致时间戳字段填入错误值。该问题存在多年之久。我们进行了多轮代码审查和测试，却始终无人发现此缺陷。单个函数内出现65个“if”分支实属过度复杂。

2) 接下来，考虑将所有输入数据验证合并为一个检查函数，并对该检查函数本身进行测试。某些程序包含多个用于接收外部输入的函数，我们称这些函数为边界函数。我们注意到边界函数可能对外部输入执行某些合理性检查，随后将控制权传递给业务逻辑函数进行数据处理。见图21-6。此方法的潜在风险在于每个边界函数都在自身函数中执行检查。若某个边界函数暴露安全问题，则需审查所有其他边界函数是否存在类似问题。遗憾的是，此过程耗时且易出错。例如，微软首个ANI漏洞（CVSS评分9.3高危，MS05-002）于2005年被发现。但修复后，相同问题（MS07-017）竟在2007年再度发现。两者根源相同，却巧合地发生在两个不同的函数中。

<div align=center><img src=Figures/Chapter-21-Screenshot/Figure-21-6.jpg></img></div>
<div align=center>图 21-6 安全边界函数的测试</div>

为降低风险，我们应将所有外部输入检查整合为单一检查函数。所有边界函数均可调用该检查函数执行数据合理性检查。我们需对该检查函数进行安全审查与安全测试，确保其具备处理所有不同情况的稳健性。若检查函数存在问题，只需在单一位置进行修复。安全测试可集中针对该检查函数开展。见图21-7。

<div align=center><img src=Figures/Chapter-21-Screenshot/Figure-21-7.jpg></img></div>
<div align=center>图 21-7 安全检查函数的测试</div>

3) 接下来，让所有外部边界函数调用检查函数并测试执行流程。建议定义一个集中化的检查函数。我们需要通过执行流程的安全性测试，确保所有边界函数在将控制权转移给业务逻辑函数之前，都调用相同的检查函数。

## 总结

在本章中，我们介绍了安全单元测试。本章涵盖了固件中八个高风险领域的安全单元测试计划 —— 外部输入、竞争条件、硬件输入、秘密处理、寄存器锁定、安全配置、重放/回滚以及密码学。随后介绍了两种高级安全单元测试方法 —— 模糊测试与符号化执行，并给出了检查函数的建议。下一章将阐述固件的安全验证与渗透测试方法论。
...
[P-6] Oleksandr Bazhaniuk, John Loucaides, Lee Rosenbaum, Mark R. Tuttle, Vincent Zimmer, “Symbolic execution for BIOS security,” in USENIX WOOT 2015, available at https://www.markrtuttle.com/data/papers/bazhaniuk-loucaides-rosenbaum-tuttle-zimmer-woot15.pdf
...
[W-1] Using HBFA to Improve Platform Resiliency, https://www.intel.com/content/dam/develop/external/us/en/documents/intel-usinghbfatoimproveplatformresiliency-820238.pdf
