...
| :--- | :--- | :--- |
| 0 | 细节：ACM策略状态（测量启动，验证启动），ACM安全版本号（SVN），ACM签名，密钥清单签名，启动策略清单签名，初始化启动块（IBB）哈希 |
| 1 | 主机固件配置 | 无 |
| 2 | UEFI驱动代码 | 无 |
| 3 | UEFI驱动配置和数据 | 无 |
| 4 | UEFI启动管理代码 | 无 |
...
coreboot也支持TCG可信启动。然而，coreboot解决方案不使用TCG平台固件概述，而是用自己的方式使用TPM替代。表7-5显示了coreboot中PCR的使用。
...
| 2 | UEFI驱动代码 | 核心测量信任根，包括所有的阶段，数据和blobs，例如COREBOOT CBFS（启动块，fallback/verstage），FW_MAIN CBFS（fallback/romstage, fspm, fallback/postcar,
fallback/ramstage, cpu_microcode_blob, fsps, vbt, fallback/dsdt.aml, fallback/payload）， RD_VPD，GBB，SI_DESC，SI_GBE |
