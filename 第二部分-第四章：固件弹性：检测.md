...
T2 MacOS安全启动链与其他安全启动解决方案类似。芯片从不可改变的Boot ROM作为信任根执行代码。然后，Boot ROM验证iBoot启动程序。后者验证T2内核，然后T2 内核验证UEFI固件。图 4-15显示了T2 MacOS安全启动链。
...
思科使用信任锚技术为思科可信系统提供基础。思科安全启动链使用了一个硬件锚点。CPU的第一条指令存储在不可改变的硬件锚中。设备启动时，微启动程序会验证启动程序，然后启动程序会验证操作系统。见图4-16。
...
AMD 为该平台引入了平台安全处理器（PSP）。PSP基于ARM TrustZone架构的。它提供了基于硬件的信任根和可信执行环境。PSP包括一个不可改变的片上Boot ROM，这Boot ROM验证片外启动程序。然后，PSP启动程序认证BIOS的第一个块，并将控制权转移给它。PSP验证流程见图4-17。
